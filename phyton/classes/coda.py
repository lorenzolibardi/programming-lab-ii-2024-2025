class Coda: # Politica First In First Out (FIFO)
    code_create = 0

    def __init__(self, n):
        """
        Metodo che viene chiamato durante la costruzione di una istanza di
        Coda per impostarne lo stato iniziale.
        """
        self.size = n
        self.items = []
        Coda.code_create += 1
    ...
    def push(self, item):
        ...
        if len(self.items) >= self.size:
            raise f"Error: queue reached max size (n={self.size})"
    ...
    def __lt__(self, altro):
        ...
        """
        assert type(altro) == Coda, "Si possono confrontare solo 2 code"
